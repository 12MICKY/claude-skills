...
The Proxmox VE cluster (`YOUR_PROXMOX_IP`) is a multi-node deployment that requires strict quorum mapping.
...
pct create <vmid> local:vztmpl/ubuntu-24.04-default_amd64.tar.zst \
  -cores 2 -memory 1024 -net0 name=eth0,bridge=vmbr0,ip=192.0.2.20/24,gw=192.0.2.1
